...
def recover() :
    ...
    try:
        ...
        logger.info("nrfjprog recover executed successfully")
    except subprocess.CalledProcessError as e:
        logger.exception("Error executing recover: %s", e)

def reset() :
    reset = "nrfjprog --reset --family NRF52  \n"
    try:
        subprocess.run(reset, shell=True, check=True)
        logger.info("nrfjprog reset executed successfully")
    except subprocess.CalledProcessError as e:
        logger.exception("Error executing reset: %s", e)

def eraseall() :
    eraseall = "nrfjprog -f NRF52 --eraseall \n"
    try:
        subprocess.run(eraseall, shell=True, check=True)
        logger.info("nrfjprog eraseall executed successfully")
    except subprocess.CalledProcessError as e:
        logger.exception("Error executing eraseall: %s", e)
        pass
...
def protection() :
    ...
    try:
        ...
        logger.info("nrfjprog protection executed successfully")
    except subprocess.CalledProcessError as e:
        logger.exception("Error executing protection: %s", e)
    pass
...
def mac_id_check(force_refresh=False):
    ...
    if not force_refresh and _mac_id_cache and (current_time - _mac_id_cache_time) < _MAC_ID_CACHE_DURATION:
        logger.debug(f"[mac_id_check] Using cached MAC ID: {_mac_id_cache} (age: {current_time - _mac_id_cache_time:.2f}s)")
    ...
    try:
        ...
        logger.info(f"[mac_id_check] Read MAC ID from device: {mac_id_local}")
        ...
        logger.warning("Cant Read macID with nrfjprog: %s", e)
        # Return cached value if available, even if stale
        if _mac_id_cache:
            logger.debug(f"[mac_id_check] Using stale cache due to error: {_mac_id_cache}")
            return _mac_id_cache
...
def clear_mac_id_cache():
    ...
    _mac_id_cache = None
    _mac_id_cache_time = 0
    logger.debug("[mac_id_check] Cache cleared")
...
def power_on():
    ...
    for exe in executables:
        try:
            ...
            logger.debug(f"{exe} returncode={proc.returncode}")
            if proc.stdout:
            ...
            logger.warning(f"{exe} timed out while powering on")
            continue
        except Exception as e:
            logger.warning(f"Error running {exe}: {e}")
            continue
    logger.warning("Failed to power on: no J-Link executable succeeded")
    return False
...
def power_off():
    ...
    for exe in executables:
        try:
            ...
            logger.warning(f"{exe} timed out while powering off")
            continue
        except Exception as e:
            logger.warning(f"Error running {exe}: {e}")
            continue
    logger.warning("Failed to power off: no J-Link executable succeeded")
    return False
...
def flash_program(hex_name):
    ...
    try:
        ...
        logger.warning("Cant Read macID with jprog: %s", e)
    return is_ok
