...
class UIThreadHelper(QObject):
    set_status_signal = pyqtSignal(object, str)
    run_error_signal = pyqtSignal(object, str)
    add_good_signal = pyqtSignal(object)
    ...
    def _set_status(self, ui, text):
        try:
            ui.flashStatusLabel.setText(text)
        except Exception:
            pass

    def _run_error(self, ui, issue):
        try:
            db_controller.error_inbetween(ui, issue)
        except Exception:
            pass

    def _add_good(self, ui):
        try:
...
_ui_thread_helper = UIThreadHelper()
...
def flash_event(ui):
    ...
    thr.start()
    _ui_thread_helper.set_status_signal.emit(ui, "Programming Status : <span style=\"color:orange\">In Progress</span></p>")
...
def flashing_thread_callback(ui):
    ...
    if result:
        _ui_thread_helper.set_status_signal.emit(ui, "Programming Status : <span style=\"color:green\">Success</span></p>")
        jlink.reset()
        _ui_thread_helper.add_good_signal.emit(ui)
    else:
        _ui_thread_helper.set_status_signal.emit(ui, "Programming Status : <span style=\"color:red\">Fail</span></p>")
        issue = 'Primary Flash Failed'
        _ui_thread_helper.run_error_signal.emit(ui, issue)
...
def afterLife_event(ui):
    ...
    thr.start()
    _ui_thread_helper.set_status_signal.emit(ui, "Status : <span style=\"color:orange\">Production In Progress</span></p>")


def afterLife_event_thread_callback(ui):
    logger.debug(ui.programFileComboBox.currentIndex()) ### Marking ###
    my_firmware_ = production_fw_path(ui)
    jlink.power_on()
    result = jlink.flash_program(os.path.join(
        "JLink/program_files/ProductionFW", my_firmware_))
    if result:
        _ui_thread_helper.set_status_signal.emit(ui, "Status : <span style=\"color:green\">Process Finish</span></p>")
        #jlink.protection()
        message = "Process Finish"
        pop_up.alert_helper.show_alert_signal.emit(message)
    else:
        _ui_thread_helper.set_status_signal.emit(ui, "Status : <span style=\"color:red\">Process Fail</span></p>")
