...
def device_update(ui,mac_id,controller_type,first_stack,second_stack,note): 
    ...
    if controller_type.startswith('Sensor Controller') :
            ...
            logger.debug(sensor_db_value)
            device_db_table = "db_sde.devices_income"
            # Upsert: UPDATE if device_id exists; otherwise INSERT
            try:
                exists_rows = db_connect().connect_select(device_db_table, f"device_id = '{mac_id}'", "device_id")
            except Exception as e:
                exists_rows = []
            if exists_rows:
                set_parts = []
                set_parts.append(f"devices_type = '{controller_type}'")
                for col, val in data:
                    set_parts.append(f"{col} = '{val}'")
                set_parts.append("print_stat = '0'")
                where = f"device_id = '{mac_id}'"
                db_connect().connect_update(device_db_table, ", ".join(set_parts), where)
            else:
                device_sensor = db_connect()
                device_sensor.connect_insert(device_db_table,sensor_db_value)
            # QR Generator --------------------------------------------------
            if len(this_mac_array) == 0  and note.startswith('GOOD'):
                    qr_gen.qr_create_path(mac_id,box_lot_id)
    #===========================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================
    elif controller_type.startswith('Actuator Controller 3CH') :
            level_all = []
            ...
            logger.debug(actuator_db_value)
            device_db_table = "db_sde.devices_income"
            # Upsert: UPDATE if device_id exists; otherwise INSERT
            try:
                exists_rows = db_connect().connect_select(device_db_table, f"device_id = '{mac_id}'", "device_id")
            ...
            if exists_rows:
                ...
                for col, val in data:
                    set_parts.append(f"{col} = '{val}'")
                set_parts.append(f"inspec_note = '{note}'")
                set_parts.append("print_stat = '0'")
                where = f"device_id = '{mac_id}'"
                db_connect().connect_update(device_db_table, ", ".join(set_parts), where)
            else:
    ...
    #============================================================
        if mac_id == port_data[0] :
                logger.warning("Duplicated Device")
                for update in data :
